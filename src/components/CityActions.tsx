...
export function CityActions({ cityName, cityId, countryId }: CityActionsProps) {
  ...
  const handleAddToVisited = async () => {
    if (!cityId) {
      toast.error('ID города не указан')
      return
    }
    ...
    try {
      const response = await fetch('/api/visited-places', {
        ...
        body: JSON.stringify({ cityId, countryId }),
      })
      ...
      toast.success(`${cityName} добавлен в посещенные места!`)
      router.refresh()
    } catch (error: any) {
  ...
  const handleAddToWishlist = async () => {
    ...
    try {
      const response = await fetch('/api/wishlist', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cityId, countryId, priority: 3 }),
      })
      ...
      toast.success(`${cityName} добавлен в желаемые направления!`)
      router.refresh()
    } catch (error: any) {
