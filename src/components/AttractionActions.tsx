...
export function AttractionActions({ attractionName, attractionId }: AttractionActionsProps) {
  ...
  const handleAddToVisited = async () => {
    if (!attractionId) {
      toast.error('ID достопримечательности не указан')
      return
    }
    ...
    try {
      ...
      toast.success(`${attractionName} добавлена в посещенные места!`)
      router.refresh()
    } catch (error: any) {
  ...
  const handleAddToWishlist = async () => {
    ...
    try {
      const response = await fetch('/api/wishlist', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ attractionId, priority: 3 }),
      })
      ...
      toast.success(`${attractionName} добавлена в желаемые направления!`)
      router.refresh()
    } catch (error: any) {
